...
const wordsList = function (str, subStr) {
    ...
    str.split(' ').forEach(string => {
        ...
        if (string.includes(subStr)) result.add(string);
    });
...
let myDate = new Date;
// new Date(42224).toLocaleString
const getLocalDate = function (myDate, isSeconds, isISO) {
    
    let localeStr = myDate.toLocaleString();

    if (isSeconds === true && isISO == undefined) return localeStr;

    if (isSeconds === false && isISO === true )
        return localeStr.slice(0, localeStr.indexOf(', '))
                .split('.').reverse().join('-').toString().concat(
                    ', ', localeStr.slice(localeStr.indexOf(', ') + 2, -3)
                );

    if (isSeconds  === true && isISO === true) 
        return localeStr.slice(0, localeStr.indexOf(', '))
                .split('.').reverse().join('-').toString().concat(
                    ', ', localeStr.slice(localeStr.indexOf(', ') + 2)
                );
    
    return localeStr.slice(0,-3);
}
...
const getLocalDay = (date) => new Date(date).getDay() === 0 ? 7 : new Date(date).getDay();
...
function testPerformance(iterations, func) {
    ...
    return (new Date().getTime() - before) / 1000;
}
